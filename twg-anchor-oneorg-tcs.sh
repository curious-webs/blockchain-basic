...
jq '.channel_group.groups.Application.groups.TCS.values += {"AnchorPeers":{"mod_policy": "Admins","value":{"anchor_peers": [{"host": "peer0.tcs.com","port": 7051}]},"version": "0"}}' channel-artifacts-anchor/${CHANNEL_NAME}_config.json > channel-artifacts-anchor/modified_anchor_${CHANNEL_NAME}.json
...
echo '{"payload":{"header":{"channel_header":{"channel_id":"oneorg", "type":2}},"data":{"config_update":'$(cat channel-artifacts-anchor/anchor_update_${CHANNEL_NAME}.json)'}}}' | jq . > channel-artifacts-anchor/anchor_update_${CHANNEL_NAME}_in_envelope.json
